#!/bin/bash
# Call like vacind_long_launch.sh chunk flirt ants asym 3 T04 T01 L modality [ID]
...
for FLAVOR in t1natnat ; do
  ./vacind_long_pipeline.sh $FLAVOR $INITTYPE $DEFTYPE $GLOBALREGPROG $USEMASK $SYMMTYPE $REGTYPE $USEDEFMASK $MASKRAD $RESAMPLE $RFIT $ASTEPSIZE $DEFREGPROG $TIMEPOINT $RIGIDMODE  $MODALITY $DOMPSUB $SEGALT $ALTMODESEG $SIDE $BLTIMEPOINT $ANTSVER $REGUL1 $REGUL2 $ALLIDS &
#  sleep 3
