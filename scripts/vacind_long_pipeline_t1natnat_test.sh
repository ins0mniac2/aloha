export side MODALITY GLOBALRIGIDPROG RIGIDMODE ASTEPSIZE METRIC GLOBALREGPROG USEMASK SYMMTYPE REGTYPE USEDEFMASK DEFREGPROG REGUL1 REGUL2
...
#   echo "Skipping global registration"
# else
...
if [ "$ANTSVER" == "v3" ] ; then
  bash ${ROOT}/scripts/bash/vacind_long_deformable_test.sh -d \
     -b ${BLGRAY} \
     -c ${BLSPACE} \
     -e ${MODALITY} \
     -f ${FUGRAY} \
