...
BIN_ANTSITK4=~srdas/bin/ants_avants
...
c3d $WDIR/futrimdef.nii.gz -origin ${BLORIG[0]}x${BLORIG[1]}x${BLORIG[2]}mm -o $WDIR/futrimdef_om.nii.gz
...
c3d_affine_tool -sform $WDIR/futrim_om.nii.gz -sform $WDIR/bltrim.nii.gz -inv -mult -o $TMPDIR/tmp.mat 
c3d_affine_tool $TMPDIR/tmp.mat -sqrt -sform $WDIR/bltrim.nii.gz -mult -o $WDIR/hwtrimspace.mat
...
# regparams="  -m Mattes[ $imgs ,1,32,Regular,0.25] -f 4x2x1 -s 2x1x0 -c [1200x1200x50,1e-08,10] "
# $reg -d 3 -r [ $imgs, 1 ]  -o [./ANTS_mutualinfo_asym,./ANTS_mutualinfo_asym_resliced.nii.gz] -t Translation[0.1] $regparams -t Rigid[0.1] $regparams -t Similarity[0.1] $regparams -t SyN[0.25,3,0] $regparams -l 1 -u 1 -w [0.0,0.995] -b 0 -z 1 
# $reg -r [ $imgs, 1 ]  -o [./ANTS_mutualinfo_asym,./ANTS_mutualinfo_asym_resliced.nii.gz] -t Translation[0.1] $regparams -t Rigid[0.1] $regparams -t Affine[0.1] $regparams -t SyN[0.25,3,0] $regparams -l 1 -u 1 -w [0.0,0.995] -b 0  -z 1 


                  # Brian's eval relative to old ANTS
# -m CC[ $imgs ,1,4] -o [ comparison/antsnew , comparison/antsnew.nii.gz ] -c [ 50x50x55, -1, 20 ] -t SyN[ 0.25 , $gauss ] -s 2.5x1.5x0.5vox -f 4x2x1 -u 1


                  # -m PR[$WDIR/bltrim_to_hw.nii.gz,$WDIR/futrim_om_to_hw.nii.gz,1,4] \
                  # -m Mattes[$WDIR/bltrim_to_hw.nii.gz,$WDIR/futrim_om_to_hw.nii.gz,1,32,Regular,0.25] \
                  # -m CC[$WDIR/bltrim_to_hw.nii.gz,$WDIR/futrim_om_to_hw.nii.gz,1,4] \
                  # -c [ $ANTSITER, -1, 20 ] \
                  # -c [ $ANTSITER, -1.e-8, 20 ] \
                  # -t SyN[ ${ASTEPSIZE?} , $REGUL ] \
                  # -t BSplineSyN[0.15,10x10x11,0x0x0,3] \
            ...
            c3d -mcs $WDIR/ants/antsreg3d1Warp.nii.gz -oo $WDIR/ants/antsreg3dWarpxvec.nii.gz $WDIR/ants/antsreg3dWarpyvec.nii.gz $WDIR/ants/antsreg3dWarpzvec.nii.gz
            c3d -mcs $WDIR/ants/antsreg3d1InverseWarp.nii.gz -oo $WDIR/ants/antsreg3dInverseWarpxvec.nii.gz $WDIR/ants/antsreg3dInverseWarpyvec.nii.gz $WDIR/ants/antsreg3dInverseWarpzvec.nii.gz
 
            # Warp followup to baseline/HW
            $BIN_ANTS/WarpImageMultiTransform 3 $WDIR/futrim_om.nii.gz $WDIR/futrim_om_to_bltrim_warped_3d.nii.gz \
            ...
            # Warp followup mask to baseline/HW as we don't want to measure where followup wasn't image. This mask will be applied to baseline segmentation in the measurements script
            # c3d $WDIR/futrim_om.nii.gz -thresh -inf inf 1 0 -o $TMPDIR/fumask.nii.gz
            # $BIN_ANTS/WarpImageMultiTransform 3 $TMPDIR/fumask.nii.gz $WDIR/futrim_om_mask_to_bltrim_warped_3d.nii.gz \
            #   -R $WDIR/bltrimdef.nii.gz --use-NN $WDIR/omRAS_half_itk.txt $WDIR/${DEFREGPROG}/${DEFREGPROG}reg3dWarp.nii.gz $WDIR/omRAS_half_itk.txt;
            # $BIN_ANTS/WarpImageMultiTransform 3 $TMPDIR/fumask.nii.gz $WDIR/futrim_om_mask_to_hw_warped_3d.nii.gz \
            #   -R $WDIR/hwtrimdef.nii.gz --use-NN $WDIR/${DEFREGPROG}/${DEFREGPROG}reg3dWarp.nii.gz $WDIR/omRAS_half_itk.txt;
            $BIN_ANTS/WarpImageMultiTransform 3 $WDIR/futrimdef_om.nii.gz $WDIR/futrimdef_om_to_bltrim_warped_3d.nii.gz \
              -R $WDIR/bltrimdef.nii.gz --use-NN $WDIR/omRAS_half_itk.txt $WDIR/${DEFREGPROG}/${DEFREGPROG}reg3dWarp.nii.gz $WDIR/omRAS_half_itk.txt;
            $BIN_ANTS/WarpImageMultiTransform 3 $WDIR/futrimdef_om.nii.gz $WDIR/futrimdef_om_to_hw_warped_3d.nii.gz \
              -R $WDIR/hwtrimdef.nii.gz --use-NN $WDIR/${DEFREGPROG}/${DEFREGPROG}reg3dWarp.nii.gz $WDIR/omRAS_half_itk.txt;
